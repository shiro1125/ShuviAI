...
3. **필수 의존성 설치:**
   ...
   *PyAudio 설치 주의:* Windows 환경에서는 PyAudio 컴파일 문제로 인해 기본 `pip install pyaudio` 명령이 실패할 수 있습니다. 이 경우 다음 명령을 차례대로 실행하여 사전 컴파일된 패키지를 설치하세요:
   ...
   이 과정이 완료되면 마이크 입력을 정상적으로 사용할 수 있습니다.

4. **환경 설정 파일 준비:**
   - `.env.example` 파일을 복사하여 `.env`로 이름을 바꿉니다.
   - `GEMINI_API_KEY`에 Google AI Studio 또는 Google Cloud Console에서 발급한 키를 입력하세요.
   - 필요하다면 `GEMINI_MODEL` 값을 원하는 모델(`models/gemini-pro` 등)로 지정합니다. 지정하지 않으면 기본값이 사용됩니다.
